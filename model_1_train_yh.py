...
input_str='62,126,13,0.8,59,125#62,126,13,0.8,59,125'
Predict_data = input_str.split('#')
fitness_list = []
for i in range(len(Predict_data)):
  results0=Predict_data[i].split(',')
  results = list(map(float, results0))
  fitness_list .append(results)
a1 = np.array(fitness_list)
# df =  pd.DataFrame(a , columns=['A', 'B', 'C','D','E','F'])
yu=a1[:, 0:np.shape(a1)[1]]
...
x_train, y_train = split_data(df2, seq_length, input_dim)
# 4.将数据转为tensor
print(type(x_train))
print(type(y_train))
...
for epoch in range(epochs):
    ...
    if running_loss < best_loss:
        ...
        recode_loss.append(running_loss)
...
print(AQI_mean)
print('y=')
prediction_np = y_test_pred.data.numpy().flatten().tolist()
for temp in range(len(prediction_np)):
    y0=prediction_np[temp]
    y = y0 * AQI_std + AQI_mean
    print(y)
